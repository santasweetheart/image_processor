 ...
 * modify model.
 */
public class RGBModel implements MaskModel {
  private Image image;
  ...
  public void makeRedMask(Image mi) {
    RGBModel adj = new RGBModel(this.getImage());
    adj.makeRed();
    Image adjusted = adj.getImage();
    MaskImpl mask = new MaskImpl();
    this.image = mask.apply(this.getImage(), mi, adjusted);
  }
  ...
  public void downscale(int width, int height) {
    Scaler d = new DownScale(width, height);
    this.image = d.applyDownscale(this.image);
  }
