...
public class GuiViewImpl extends JFrame implements GuiView {
  ...
  private void buildDownScale() {
    JButton downscale = new JButton("Downscale");
    downscale.addActionListener((ActionEvent e) -> {
      if (commandCallback != null) {
        String dimensions = JOptionPane.showInputDialog(this,
            "Insert the dimensions. Width then height (e.g 200 300)");
        if (dimensions != null && dimensions.length() > 0) {
          commandCallback.accept("downscale " + dimensions);
        }
      }
    });
