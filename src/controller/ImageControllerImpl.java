...
public class ImageControllerImpl implements ImageController {
      ...
      if (input.hasNext()) {
        ...
        if (nextInput.equalsIgnoreCase("q") ||
          ...
          if (validCommand(nextInput)) {
            if (nextInput.equals("downscale")) {
              int width = input.nextInt();
              int height = input.nextInt();
              downScaleHandler(width,height);
            } else {
  ...
  private void menu() {
    ...
    tryCatchRenderMessage("To apply an operation using a mask type the location of the mask"
            + "image after the filepath of the current image");
  ...
  private void downScaleHandler(int width, int height) {
    new DownScaling(width, height).run();
  }
  ...
  private class DownScaling implements Runnable {
    int width;
    int height;
    ...
    public void run() {
      ...
      tryCatchRenderMessage("Downscaling Image....");
    }
  }
